...
const VERY_IN_LOVE_WORLD_SCENES = [
  createScene("very-love-smile", "muy-enamorado", "💖", "Modo Reporterito Muy Enamorado", "Todas las barras están arriba del 90%. El reporterito no puede dejar de sonreír y el cuarto se llenó de brillitos rosas.", "happy", 4, { veryInLove: true }),
  createScene("very-love-pancho", "muy-enamorado", "🐶", "Pancho quiere jugar más", "Pancho notó que el reporterito está muy feliz, agarró su juguete y empezó una mini celebración en el cuarto.", "panchito", 3, { veryInLove: true, panchito: true }),
  createScene("very-love-tree", "muy-enamorado", "🌳", "El árbol respondió al amor", "El Árbol de Amor soltó hojitas de corazón porque el reporterito se siente demasiado cuidado por la Directiva.", "stars", 5, { veryInLove: true }),
  createScene("very-love-memory", "muy-enamorado", "📖", "Recuerdo premium", "El reporterito guardó este momento como prueba oficial de que cuando la Directiva lo cuida, su mundo se vuelve más bonito.", "photo", 4, { veryInLove: true })
];
...
const ONLINE_AWAKE_LOSS_PER_20_MIN = 2;
const SLEEP_LOSS_PER_HOUR = 2;
const VERY_IN_LOVE_THRESHOLD = 90;

function isVeryInLoveMode(sourceState = state) {
  if (!sourceState || sourceState.life?.isDead) return false;
  return Object.values(sourceState.stats || {}).every(value => Number(value) >= VERY_IN_LOVE_THRESHOLD);
}

function veryInLoveMultiplier(sourceState = state) {
  return isVeryInLoveMode(sourceState) ? 0.5 : 1;
}

function calculateOfflineDecay(lastSaved, now = Date.now(), multiplier = 1) {
  ...
  while (cursor < now) {
    ...
    if (isReporterSleepingNow(new Date(cursor))) {
      loss += SLEEP_LOSS_PER_HOUR * hours;
      sleptMs += next - cursor;
    } else {
      loss += ONLINE_AWAKE_LOSS_PER_20_MIN * ((next - cursor) / step);
      awakeMs += next - cursor;
    }
    cursor = next;
  }
  return { loss: Math.min(100, loss * multiplier), sleptMs, awakeMs };
}

function applyOfflineDecayToMergedState(merged, lastSaved) {
  const { loss, sleptMs, awakeMs } = calculateOfflineDecay(lastSaved, Date.now(), veryInLoveMultiplier(merged));
...
function addTreeXP(amount) {
  const basePoints = Math.max(0, Number(amount) || 0);
  if (!basePoints) return;
  const points = Math.ceil(basePoints * (isVeryInLoveMode() ? 2 : 1));
...
function sleepBlockedMessage() {
  const text = "El reporterito está durmiendo de 12:00 a.m. a 8:00 a.m. 😴 Zzz... No puede hacer acciones, responder mensajes ni abrir escenas hasta que despierte. Sus barras bajan suave: 2% por hora.";
  setMessage(text);
...
function contextualScenePool() {
  ...
  if (state.life.mood === "panchitoMode" || Math.random() < .3 || isVeryInLoveMode()) pool.push(...PANCHITO_WORLD_SCENES);
  if (isVeryInLoveMode()) pool.push(...VERY_IN_LOVE_WORLD_SCENES, ...VERY_IN_LOVE_WORLD_SCENES);
...
function resolveWorldScene(index) {
  ...
  if (option[4] || currentWorldScene.veryInLove) addAlbumMemory(option[1], currentWorldScene.veryInLove ? "MODO MUY ENAMORADO" : "ESCENA DEL MUNDO", currentWorldScene.icon, `world-scene-${currentWorldScene.id}-${Date.now()}`);
...
function updateRoomState(mood) {
  ...
  else if (isVeryInLoveMode()) visual = Math.random() < .65 ? "happy" : "photo";
...
function cyclePanchitoBehavior() {
  ...
  else if (isVeryInLoveMode()) setPanchitoVisual(randomFrom(["playing", "happy", "playing", "happy", "idle"]));
...
function render(forceMessage = false) {
  ...
  if (isVeryInLoveMode() && !dead && Date.now() - Number(state.life.lastVeryInLoveNotice || 0) > 30 * 60 * 1000) {
    state.life.lastVeryInLoveNotice = Date.now();
    setMessage("💖 Modo Reporterito Muy Enamorado activado: desgaste al 50%, más escenas, Pancho más juguetón y XP duplicada.");
    toast("💖", "MODO MUY ENAMORADO", "El reporterito está brillando de amor.");
  } else if (forceMessage || newMood !== currentMood) setMessage(randomFrom(moodMessages[newMood]));
  currentMood = newMood;
...
function renderWeather(mood) {
  ...
  if (["triste", "muy-triste"].includes(mood)) {
    ...
    const hearts = isVeryInLoveMode() ? 16 : 8;
    layer.innerHTML = Array.from({ length: hearts }, (_, i) => `<i class="sky-heart" style="left:${i * 13}%;animation-delay:-${i}s">♥</i>`).join("");
...
function decay() {
  if (state.life.isDead) { renderLifeCounter(); return; }
  const sleeping = isReporterSleepingNow();
  const baseLossPerTick = sleeping ? SLEEP_LOSS_PER_HOUR / (3600000 / 5000) : ONLINE_AWAKE_LOSS_PER_20_MIN / ((20 * 60 * 1000) / 5000);
  const inLoveProtection = veryInLoveMultiplier();
  const lossPerTick = baseLossPerTick * inLoveProtection;
  ...
  if (sleeping) {
    showLifeEventAlert(false);
    if (decayCycles % 12 === 0) setMessage("El reporterito está durmiendo. Zzz... De 12:00 a.m. a 8:00 a.m. no puede hacer acciones ni abrir escenas. Sus barras bajan suave: 2% por hora.");
  } else {
